...
terminal_commands = {
    ...
    6: {
        'name': 'Save PCAP File',
        'start': 'tshark -i eth0 -w "/home/nvidia/FLASK_CSD/PCAP_tshark/capture_$(date +%Y%m%d_%H%M%S).pcap"',
        'delay': 1
    }
}
    ...
    while True:
        try:
            ...
            if output:
                ...
                if output_text:
                    # Add to output queue
                    output_queues[terminal_id].put(output_text)
...
if __name__ == '__main__':
    ...
    socketio.run(app, host='0.0.0.0', port=5000, debug=True, allow_unsafe_werkzeug=False)