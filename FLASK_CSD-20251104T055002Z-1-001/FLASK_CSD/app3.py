# app.py
#eventlet is very important to monitor ans send realtime data data at super fast speed
#socketio is far better than pooling here it is udated from pooling to socketio
import eventlet
...
#import eventlet
#eventlet.monkey_patch() # eventlet monkey should be patched at 1st before importing flask not last 
# Set up logging
...
socketio = SocketIO(app, cors_allowed_origins="*", ping_timeout=5, ping_interval=2, logger=False, engineio_logger=False)
# Do logger false if want to print less line in shell
sys.stdout.reconfigure(line_buffering=True)  # Python 3.7+; ensures prints are flushed automatically
    ...
    while True:
        try:
            ...
            try:
                ...
                sys.stdout.flush()  # force Python to flush immediately
                if output:
                     # output_text = output.decode('utf-8', errors='ignore').strip() 
                   # this line was reason why at html end we aere not getting any output
                    output_text = output.strip()   # already str, no decode
                    if output_text:
                        # Store the output
                        ...
                        socketio.sleep(0)  # yield to event loop # this line has been added to reduce delay at html terminal output 
...
def start_terminal(id):
    ...
    try:
        ...
        #threading.Thread(target=read_process_output, args=(process, id), daemon=True).start() # eventlet works fine with socketio so threding.thread is not use
        # Instead of threading.Thread(...).start()
        socketio.start_background_task(read_process_output, process, id) # instead of threading.thread it is used which can handle socket.io. sleep(0)
...
@app.route('/')
def index():
    return render_template('index03.html', terminals=terminal_commands)
