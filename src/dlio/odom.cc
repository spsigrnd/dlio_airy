...
void dlio::OdomNode::getScanFromROS(const sensor_msgs::PointCloud2ConstPtr& pc) {
  ...
  for (auto &field : pc->fields) {
    if (field.name == "t") {
      ...
      break;
    } else if (field.name == "timestampmakeunknow" && original_scan_->points[0].timestamp < 1e14) {
    ...
    } else if (field.name == "timestamp" && pc->header.frame_id.find("rslidar") != std::string::npos) {
...
void dlio::OdomNode::deskewPointcloud() {
  ...
  if (this->sensor == dlio::SensorType::OUSTER) {
    ...
    extract_point_time = [&sweep_ref_time](boost::range::index_value<PointType&, long> pt)
      {
        // RoboSense Airy: µs → seconds
        return sweep_ref_time + pt.value().timestamp * 1e-6;}; 
   }
  ...
  if (!this->imu_calibrated) {
    ...
    if ((imu->header.stamp.toSec() - this->first_imu_stamp) < this->imu_calib_time_) {
      ...
      if (this->gravity_align_) {
        ...
        std::cout << "grav_vec = " << grav_vec.transpose() << std::endl;
        std::cout << "accel_avg = " << accel_avg.transpose() << std::endl;
        std::cout << "state.b.accel = " << this->state.b.accel.transpose() << std::endl;
    ...
    //std::cout << "lin accel corrected = " << lin_accel_corrected.transpose() << std::endl;
...
void dlio::OdomNode::debug() {
  ...
  if (this->sensor == dlio::SensorType::OUSTER) {
    ...
    std::cout << "| " << std::left << std::setfill(' ') << std::setw(66)
      << "Sensor Rates: RoboSense @ " + to_string_with_precision(avg_lidar_rate, 2)
                                     + " Hz, IMU @ " + to_string_with_precision(avg_imu_rate, 2) + " Hz"
      << "|" << std::endl;
